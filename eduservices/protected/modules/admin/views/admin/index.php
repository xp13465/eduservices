<?php
		...
		$checkNUM=Students::model()->getNum($UArr,2,1,"",true);
		$OkNUM=Students::model()->getNum($UArr,2,2,"",true);
        $bhNUM=Students::model()->getNum($UArr,2,3,"",true);
        
        $SMcheckNUM=Students::model()->getNum($UArr,1,1,"",true);
        $SMOkNUM=Students::model()->getNum($UArr,1,2,"",true);
        $SMbhNUM=Students::model()->getNum($UArr,1,3,"",true);
        //排考控制器里的内容
        $criteria=new CDbCriteria;
...
<table class="table table-bordered info" width="100%">
    ...
    <tbody>
    <tr>
        <td width="50%">学员列表人数 <font color="#0088CC"><?=$bhNUM+$checkNUM+$OkNUM?></font></td>
