	...
	{ 
		...
		foreach($_GET as $key=>$val){
            ...
            if($key=="marktype"&&$val){
                ...
                if($val==1){
                    if(isset($_GET["score_start"])&&$_GET["score_start"]>=0)$acriteria->addCondition("sc_kgmark  >= '{$_GET["score_start"]}' ");
                    if(isset($_GET["score_end"])&&$_GET["score_end"]>=0)$acriteria->addCondition("sc_kgmark  <= '{$_GET["score_end"]}' ");
                 }
                // if($key=="score_end"&&$val>=0)$criteria->addCondition("sc_kgmark <= '{$val}' ");
