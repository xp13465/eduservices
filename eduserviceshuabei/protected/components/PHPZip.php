...
class PHPZip{
     ...
     * @param <array> $files
     */
    public function downloadZipByFiles($files){
        $this->createZipByFiles($files);
    }
    private function createZipByFiles($files){
        if (@function_exists('gzcompress')){
            @set_time_limit("0");
            foreach($files as $dir){
                $fd = fopen ($dir, "r");
                $fileValue = fread ($fd, filesize ($dir));
                $filename = basename($dir);
                fclose ($fd);
                $this -> addFile($fileValue, "$filename");
            }
            $zipfilenametemp = time().".zip";
            $out = $this -> filezip();
            ...
            if ($filesize < 104857600) {
                ...
                header("Content-disposition: attachment; filename=".$zipfilenametemp);
                echo $this -> filezip();
            }else{
