 ...
 * 作用：将汉字转换为拼音
 *用法：
 *第二个参数设置为gb2312 则使用gb2312编码
 */
class Pinyin{
    /**
     *执行汉字转换
     * @param <int> $_String 输入的内容
     * @param <int> $_Code 编码 默认UTF-8
     * @return <array> array("long","short")
     */
    public function doWord($_String, $_Code='utf-8')
    {
        ...
        for($i=0; $i<strlen($_String); $i++)
        {
            ...
            $oneWord = $this->_Pinyin($_P, $_Data);
            $long .= $oneWord;
            $short .= substr($oneWord, 0, 1);
        }
...
//            $_String .= chr(0x80 | $_C & 0x3F);
//        }
        $_String = $_C;//调用上面的方法，在遇到类似335、1788国际中心这类的时候也不行。因此注释掉。
