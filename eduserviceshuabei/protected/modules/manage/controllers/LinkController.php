...
class LinkController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Link']))
		{
            ...
            if($model->validate()){
                if(isset($_FILES['Link']["name"]["logo"])&&!empty ($_FILES['Link']["name"]["logo"])){
                    $originfilename = $_FILES['Link']['name']['logo']; //文件名
                    $fileext = strtolower(substr($originfilename,strrpos($originfilename,'.'))); //后缀名
                    //验证文件格式是否正确
                    $fileName = "linkpic/".time().rand("00", "99").$fileext;
                    $targetFile =  PIC_PATH.$fileName;
                    $tempFile = $_FILES['Link']['tmp_name']['logo'];
                    move_uploaded_file($tempFile,$targetFile);
                    $model->logo = PIC_URL.$fileName;
                }
                if($model->save())
